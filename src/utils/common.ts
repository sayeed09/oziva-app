...
export const updateUserMoEngageProfile = userDetails => {
  ...
  if (userDetails?.customerId)
    ReactMoE.identifyUser(userDetails?.customerId);
...
export const getWhiteListHeaders = () => {
  if (process.env.EXPO_AUTOMATION_REQUEST && process.env.EXPO_WHITE_LIST_STAGING) {
    return {
      'X-Request-Purpose': process.env.EXPO_WHITE_LIST_STAGING,
      'x-automation-request': process.env.EXPO_AUTOMATION_REQUEST
    }
  } else {
    return null
  }
...
export const compareAppVersions = (version1, version2) => {
  ...
  for (let i = 0; i < Math.max(v1Parts.length, v2Parts.length); i++) {
    ...
    if (p1 > p2) return true; // version1 is greater
    if (p1 < p2) return false; // version2 is greater
  }
